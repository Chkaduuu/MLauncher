...
function initRAMInput(maxRamSystem: number) {
  ...
  const maxRam = Math.min(maxRamSystem, 32)
  ramInput.max = maxRam.toString()

  if (ramHint) {
    ramHint.innerText = `Max available: ${maxRam} GB. Recommended: 4 GB or more.`
  }

  ramInput.addEventListener('input', () => {
    let val = parseFloat(ramInput.value)
    if (val < 1) ramInput.value = '1'
    if (val > maxRam) ramInput.value = maxRam.toString()
  })
}
...
function initFormValues(resolution: { width: number; height: number }) {
  ...
  // Берём max значение как основное (например "4G" -> 4)
  if (ramInput) {
    const maxVal = parseFloat(currentSettings.memory.max.replace('G', ''))
    ramInput.value = isNaN(maxVal) ? '4' : maxVal.toString()
  }
...
async function saveSettings() {
  ...
  const ramVal = parseFloat(ramInput?.value ?? '4')
  const minRam = Math.max(1, Math.floor(ramVal / 2)) // min = половина от max, минимум 1
  const maxRam = ramVal
